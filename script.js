...
function getRoadColor(fclass) {
  switch (fclass) {
    ...
    case 'primary_link':
      return '#ff4d4d';
    case 'primary': // هذا التكرار قد يكون خطأ إملائي، هل تقصد فئة أخرى؟
      return '#ffa500';
    case 'unclassified':
      return '#28a745';
    case 'residential':
...
function generateMapLegendControl() {
    ...
    legend.onAdd = function (map) {
        ...
        roadsLayer.eachLayer(layer => {
            ...
            console.log("Processing road layer. fclass found:", fclass); // <--- أضف هذا السطر
            uniqueRoadClasses.add(fclass);
        });

        console.log("Unique road classes collected:", Array.from(uniqueRoadClasses)); // <--- أضف هذا السطر
